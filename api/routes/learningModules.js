...
router.post("/course/add", auth, (req, res) => {
  ...
  const course_id =
    course_information.available_courses[last_data].course_id + 1;
...
router.post("/course/add/material", auth, (req, res) => {
  ...
  if (!material_type || !material_media) {
    return res.status(400).json({
      ...
      error: true,
      message:
        "Bad request please specify the course ID, material type, and material media",
    });
  }
  ...
  if (index < 0) {
    ...
    const total_data =
      course_information.available_courses[index].material.length;
    let material_ID;

    if (total_data != 0) {
      material_ID =
        course_information.available_courses[index].material[total_data - 1]
          .material_id + 1;
    } else {
      material_ID = 1;
    }
...
router.post("/course/add/lecture", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    const total_data =
      course_information.available_courses[index].lectures.length;
    let lecture_ID;

    if (total_data != 0) {
      lecture_ID =
        course_information.available_courses[index].lectures[total_data - 1]
          .lectures_id + 1;
    } else {
      lecture_ID = 1;
    }
...
router.post("/course/add/quiz", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (total_data != 0) {
      quiz_ID =
        course_information.available_courses[index].quiz[total_data - 1]
          .quiz_id + 1;
    } else {
      quiz_ID = 1;
    }
...
function FindQuizIndex(course_index, quiz_ID) {
  for (
    let i = 0;
    i < course_information.available_courses[course_index].quiz.length;
    i++
  ) {
    if (
      course_information.available_courses[course_index].quiz[i].quiz_id ==
      quiz_ID
    ) {
      return i;
    }
...
router.post("/course/add/quiz/question", auth, (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      ...
      error: true,
      message:
        "Bad request please specify the question text, question answers, and the question answer options",
    });
  }
  ...
  if (index < 0) {
    ...
    if (quiz_index < 0) {
      ...
      const total_data =
        course_information.available_courses[index].quiz[quiz_index].questions
          .length;
      let question_num;

      if (total_data != 0) {
        question_num =
          course_information.available_courses[index].quiz[quiz_index]
            .questions[total_data - 1].question_number + 1;
      } else {
        question_num = 1;
      }
      ...
      course_information.available_courses[index].quiz[
        quiz_index
      ].questions.push(new_quiz);
...
router.post("/course/update", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (course_last_updated) {
      course_information.available_courses[index].course_last_updated.value =
        course_last_updated;
      success = true;
    }
    ...
    if (success == true) {
      ...
      return res.status(400).json({
        ...
        error: true,
        message:
          "Bad request please specify the course ID, course name, and category type",
      });
    }
  }
});
...
function FindMaterialIDIndex(course_index, material_ID) {
  for (
    let i = 0;
    i < course_information.available_courses[course_index].material.length;
    i++
  ) {
    if (
      course_information.available_courses[course_index].material[i]
        .material_id == material_ID
    ) {
      return i;
    }
...
router.post("/course/update/material", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (materialIndex < 0) {
      ...
      if (material_type) {
        course_information.available_courses[index].material[
          materialIndex
        ].material_type = material_type;
        success = true;
      }
      ...
      if (success == true) {
        ...
        return res.status(400).json({
          ...
          error: true,
          message:
            "Bad request please specify the course ID, material ID, material type, and material media",
        });
      }
    }
  }
...
function FindLectureIDIndex(course_index, lecture_ID) {
  for (
    let i = 0;
    i < course_information.available_courses[course_index].lectures.length;
    i++
  ) {
    if (
      course_information.available_courses[course_index].lectures[i]
        .lectures_id == lecture_ID
    ) {
      return i;
    }
...
router.post("/course/update/lecture", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (lectureIndex < 0) {
      ...
      if (lecture_type) {
        course_information.available_courses[index].lectures[
          lectureIndex
        ].lectures_type = lecture_type;
...
router.post("/course/update/quiz", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (quizIndex < 0) {
      ...
      if (maxTries) {
        course_information.available_courses[index].quiz[quizIndex].max_tries =
          maxTries;
        success = true;
      }
...
function FindQuestionNumberIndex(course_index, quiz_index, question_number) {
  for (
    let i = 0;
    i <
    course_information.available_courses[course_index].quiz[quiz_index]
      .questions.length;
    i++
  ) {
    if (
      course_information.available_courses[course_index].quiz[quiz_index]
        .questions[i].question_number == question_number
    ) {
      return i;
    }
  }
...
router.post("/course/update/quiz/question", auth, (req, res) => {
  ...
  if (index < 0) {
    ...
    if (quizIndex < 0) {
      ...
      const quesNumIndex = FindQuestionNumberIndex(
        index,
        quizIndex,
        question_number
      );
      ...
      if (quesNumIndex < 0) {
        ...
        if (question_text) {
          course_information.available_courses[index].quiz[quizIndex].questions[
            quesNumIndex
          ].question_text = question_text;
          success = true;
        }
        ...
        if (option_A) {
          course_information.available_courses[index].quiz[quizIndex].questions[
            quesNumIndex
          ].question_answer_options.A = option_A;
          success = true;
        }
        if (option_B) {
          course_information.available_courses[index].quiz[quizIndex].questions[
            quesNumIndex
          ].question_answer_options.B = option_B;
          success = true;
        }
        if (option_C) {
          course_information.available_courses[index].quiz[quizIndex].questions[
            quesNumIndex
          ].question_answer_options.C = option_C;
          success = true;
        }
        if (option_D) {
          course_information.available_courses[index].quiz[quizIndex].questions[
            quesNumIndex
          ].question_answer_options.D = option_D;
          success = true;
        }
...
router.get("/sort-newest-module", auth, (req, res) => {
  ...
  for (let i = 0; i < course_information.available_courses.length; i++) {
    dates.push(
      course_information.available_courses[i].course_last_updated.value
    );
  }
  ...
  for (let i = 0; i < sortedIndices.length; i++) {
    sortedNewestModule.push(
      course_information.available_courses[sortedIndices[i]]
    );
  }
...
router.get("/:courseID/quiz", auth, (req, res) => {
  ...
  const filteredCourseQuizzes = course_information.available_courses.find(
    (c) => {
      return c["course_id"] === Number(courseID);
    }
  );
