...
const addTag = async (req, res) => {
  ...
  const similarTag = course_information.available_courses[
    courseIdIndex
  ].course_tag.find((tag) => tag === newTag);
...
const deleteTag = async (req, res) => {
  ...
  await axios
    .get("http://127.0.0.1:443/api/learningModules")
    .then((response) => {
      const course = response.data.available_courses.find(
      ...
      const courseTag = course.course_tag.find(
        (courseTag) => courseTag === deleteTag
      );
      ...
      if (courseTag === undefined) {
        ...
        course_information.available_courses[courseIdIndex].course_tag.pop(
          deleteTag
        );
